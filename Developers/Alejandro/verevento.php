...
<form action="cancelarevento.php" method="post" id="usrform"> 
			...
			$result = mysqli_query($conn, 'SELECT *
									       ...
									       WHERE O.USU_RUT ="'.$rut.'"
									         AND E.EVE_CORREL = '.$value.';');
			
			while($row = mysqli_fetch_array($result)){

					$usu_rut = $row["USU_RUT"];
					$eve_correl = $row["EVE_CORREL"];
 	
			}

			if($rol==1 || $rol==2 && $rut == $usu_rut && $value == $eve_correl){
			?>

				<form name="formasist" action="asistencia.php?evento=<?php echo $value ?>" method="post">
					<input type="submit" name="Ver asistencia">
				</form>
